// Heap Data Member
// демонстрация поведения объекта, имеющего член данных, память для которого выделяется динамически
...
public:
	...
	~Critter(); // Д
	Critter(const Critter& с); // КК
	Critter& Critter::operator = (const Critter& с); // ОП
	void Greet() const;
private:
	std::string* m_pName;
	int m_Age;
