...
#include "winapi_sinusoid_and_rose.h"
...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow) {
	...
	LoadString(hInstance, IDC_WINAPI_SINUSOID_AND_ROSE, szWindowClass, MAX_LOADSTRING);
	...
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_WINAPI_SINUSOID_AND_ROSE));
...
ATOM MyRegisterClass(HINSTANCE hInstance) {
	...
	wcex.hIcon			= LoadIcon(hInstance, MAKEINTRESOURCE(IDC_WINAPI_SINUSOID_AND_ROSE));
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
	wcex.lpszMenuName	= MAKEINTRESOURCE(IDC_WINAPI_SINUSOID_AND_ROSE);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_CREATE: //Создаем перья
		hpen1 = CreatePen(PS_DOT,4,RGB(0, 0, 255));
		hpen2 = CreatePen(PS_DOT,4,RGB(255, 0, 0));
		break;
